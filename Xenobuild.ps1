...
if ($mode -eq 'installdeps') {
    cmake -B"$CATCH2_BUILD_DIR" -S"$CATCH2_SOURCE_DIR" -G "$CMAKE_GENERATOR" -DCMAKE_INSTALL_PREFIX="$CATCH2_INSTALL_PREFIX" -DCATCH_BUILD_TESTING=OFF -DCATCH_INSTALL_DOCS=OFF
    ...
    # build glfw
    cmake -B"$GLFW3_BUILD_DIR" -S"$GLFW3_SOURCE_DIR" -G "$CMAKE_GENERATOR" -DCMAKE_BUILD_TYPE="$CMAKE_BUILD_TYPE" -DCMAKE_INSTALL_PREFIX="$GLFW3_INSTALL_PREFIX" -DGLFW_BUILD_DOCS=OFF -DGLFW_BUILD_EXAMPLES=OFF -DGLFW_BUILD_TESTS=OFF
