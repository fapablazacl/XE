...
inline std::string to_str(const aiTextureType type) {
    ...
    default: return "Uknnown aiTextureType enumeration value: " + std::to_string(static_cast<int>(type));
    }
}
...
template<typename T>
T extract(std::optional<T> value, const T defaultValue) {
...
Material createMaterial(const std::string &parentPath, Renderer &renderer, TextureRepository &textureRepository, const aiMaterial *aimaterial, const std::vector<GLuint> &textures) {
    if (!aimaterial) {
        return {};
    }
...
std::vector<Material> createMaterialArray(const std::string &parentPath, Renderer &renderer, TextureRepository &textureRepository, const aiScene *aiscene, const std::vector<GLuint> &textures) {
