...
namespace Sandbox {
    ...
    public:
        explicit SandboxApp(int argc, char **argv) { sceneDescription = createSceneDescription(argc, argv); }
    ...
    private:
        ...
        void renderMatrices(const XE::Matrix4f &model = XE::M4::identity()) {
            ...
            const Matrix4f modelViewProj = XE::transpose<float, 4, 4>(Matrix4f::perspective(mCamera.fov, mCamera.aspectRatio, mCamera.znear, mCamera.zfar) *
                                                                      Matrix4f::lookAtRH(mCamera.position, mCamera.lookAt, mCamera.up) * Matrix4f::rotateY(radians(m_angle)));
    ...
    std::unique_ptr<Application> Application::create(int argc, char **argv) { return std::make_unique<SandboxApp>(argc, argv); }
} // namespace Sandbox
