...
namespace Sandbox {
    ...
    SandboxApp::SandboxApp(int argc, char **argv) { sceneDescription = createSceneDescription(argc, argv); }
    ...
    bool SandboxApp::ShouldClose() const { return m_shouldClose; }
    ...
    void SandboxApp::renderMatrices(const XE::Matrix4f &) {
    ...
    Texture2D *SandboxApp::createColorTexture(const int width, const int height, const Vector4f &color) {
    ...
    Texture2D *SandboxApp::createFileTexture(const std::string &filePath) {
    ...
    SceneDescription SandboxApp::createSceneDescription(int argc, char **argv) {
        assert(argc > 0);
        assert(argv);
    ...
    void SandboxApp::initializeShaders() {
    ...
    std::string SandboxApp::loadTextFile(const std::string &path) const {
        std::fstream fs;
    ...
    void SandboxApp::initializeGeometry() {
    ...
    std::map<std::string, GeoObject> SandboxApp::loadGeoObject(const std::string &sceneFilePath) {
    ...
    std::pair<Subset *, SubsetEnvelope> SandboxApp::createSubset(const MeshPrimitive &meshPrimitive) {
