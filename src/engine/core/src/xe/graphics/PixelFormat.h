...
namespace XE {
    ...
    inline std::tuple<int, int, int, int> get_color_bit_count(const PixelFormat format) {
        switch (format) {
        case PixelFormat::R8G8B8: return { 8, 8, 8, 0};
        case PixelFormat::R8G8B8A8: return { 8, 8, 8, 8};
    ...
    inline int get_depth_bit_count(const DepthFormat format) {
        switch (format) {
        case DepthFormat::D24: return 24;
        case DepthFormat::D32: return 32;
        default: return 0;
        }
    }
    ...
    inline int get_stencil_bit_count(const StencilFormat format) {
        switch (format) {
        case StencilFormat::SUnused: return 0;
        case StencilFormat::S8: return 8;
        default: return 0;
        }
    }
}
